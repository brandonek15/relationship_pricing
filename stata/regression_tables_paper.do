...
foreach table_type in  simple controls {
	...
	if "`table_type'" == "simple" {
		local rhs rel_*  i_discount_1_simple* mi_discount_1_simple* 
		local table_suffix_add 
	}
	if "`table_type'" == "controls" {
		local rhs rel_*  i_discount_1_controls* mi_discount_1_controls* 
	...
	foreach table in sdc ds {
		...
		esttab est* using "$regression_output_path/regressions_inten_ds_chars_`table'`table_suffix_add'_paper.tex", ///
		replace  b(%9.3f) se(%9.3f) r2 label nogaps compress star(* 0.1 ** 0.05 *** 0.01) keep(i_discount_1_*_rev i_discount_1_*_b_term) ///
		title("Likelihood of hiring after relationships") scalars("fe Fixed Effects" "sample Sample" ) ///
		addnotes("Table suppresses past relationship indicators" "Obsersvation is SDC deal x lender or DS loan x lender" "Sample is 20 largest lenders x each deal/loan" ///
		"Hire indicator either 0 or 100 for readability" "SEs clustered at firm level" )
	}
}
...
foreach table in sdc ds {
	...
	foreach type in  `lhs'  {
		...
		reghdfe `lhs' rel_* `rhs_add' `cond' & hire !=0, absorb(`absorb') vce(cl cusip_6)
		estadd local fe = "`fe_local'"
		estadd local sample = "`scalar_label'"
		estimates store est`i'
		local ++i
	...
	esttab est* using "$regression_output_path/regressions_exten_pricing_rel_`table'_paper.tex", ///
	replace  b(%9.3f) se(%9.3f) r2 label nogaps compress star(* 0.1 ** 0.05 *** 0.01) drop(_cons `drop_add') ///
...
foreach table in sdc ds {
	...
	esttab est* using "$regression_output_path/regressions_exten_pricing_ds_chars_`table'_paper.tex", ///
	replace  b(%9.3f) se(%9.3f) r2 label nogaps compress star(* 0.1 ** 0.05 *** 0.01) keep(i_discount_1_*_rev i_discount_1_*_b_term) ///
	title("Pricing After Previous Loan Characteristics") scalars("fe Fixed Effects" "sample Sample" ) ///
	addnotes("Table suppresses past relationship indicators" ///
